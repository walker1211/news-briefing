...
`新闻财经` 示例配置同时覆盖国内财经、全球股市、利率、美元、美债和黄金：Bloomberg Markets 提供市场报道，美联储货币政策 RSS 提供一手公告，Investing Commodities 补充黄金及大宗商品线索（按转载来源处理）。三者的每轮入选上限分别为 6、3、4 条，共享原有分类总预算；入选候选仍需经过摘要去重和重要性终审，不保证每期都有各主题内容。

扩展主题时优先修改 `filters.categories.<分类>.include_keywords` / `weak_keywords`，并检查来源级覆盖：匹配规则优先级为 `filters.sources.<来源>.include_keywords`、`sources[].keywords`、分类规则、全局 `keywords`，不是将它们全部合并。一个强关键词或至少两个弱关键词可入选；财经排除词针对荐股等低信息量内容，不再用“盘中”“日内”“涨停”“跌停”整体屏蔽重大市场波动。新增来源应先验证 RSS 可解析、文章时间与筛选效果；原有时间窗口和去重规则照常生效。

单个来源的 `max_articles` 在时间与关键词筛选后按确定性分数截取，不再只取最新文章。分类限额复用同一评分：

- 相关性：标题每个强关键词 +120、摘要 +50；弱关键词分别 +35、+15，总计最多 400。同一关键词在每个字段只计一次，HTML 标签及属性不参与评分。
- 安全/隐私事件：标题命中代码或数据泄露、未经授权上传、漏洞等固定信号 +600；“越狱”“入侵”还须有数字技术语境。同一标题同时有官方回应、致歉或修复等跟进信号，再 +100。多次命中不叠加，单独出现“回应”不加分。
- 时效：以同一来源（分类截取时为同一分类）的最新候选为基准，最新 +100，每早一个完整小时减 5，最低 0。
- 综合早报/日报/周报等固定标题前缀扣 450，且不获得安全事件加分，避免合集挤占独立报道。

来源内按总分、发布时间降序稳定选取，输出仍保持时间顺序。分类阶段额外保留原有来源 `priority × 5`、近重复标题扣分与补发优先机制。分数用于候选排序，不是事实可信度、严重程度评级或最终发布保证；时间窗口、排除词、去重、摘要终审和 XHS 发布规则仍独立生效。权重为代码内固定规则，不额外调用 AI。
