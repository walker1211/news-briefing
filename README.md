...
Each `.source-stats.json` sidecar records per-source fetch duration, response bytes, cache status, AI input count, and final selection count. For structured briefings, only main-feed articles referenced by final stories through `source_article_ids` are written to `seen.json`. Keyword-matched candidates omitted by either the category editor or final editor remain eligible for a later window that still covers their publication time. Watch keeps its separate state store and is unaffected.
...
`output.xhs_preselection` optionally prepares an XHS-only card-manifest selection. Email and Markdown keep the final editor's original `stories`; the manifest first keeps eligible email-selected stories in their original order. With `email_only: true`, selection stops there and `target_items` is only a maximum, so the manifest may contain fewer stories rather than supplementing the email selection. Otherwise, the selector ranks only low-risk technology stories from the configured category-worker candidate pools to backfill up to `target_items`. A backfill must describe a technology product, model, developer tool, robot, chip, or compute topic, avoid public-safety and geopolitical terms, and have either a direct primary/official source or corroborated reporting. If too few candidates satisfy that gate, the manifest deliberately contains fewer than `target_items` stories. Eligible backfills are ranked as stories—not individual articles—using technology relevance, source evidence, direct-source provenance, and recency relative to the eligible backfill pool; equal scores preserve the candidate input order. Each manifest item records whether it came from the email selection or backfill; backfills also record their score components for downstream audit. `excluded_keywords` is an optional, case-insensitive hard exclusion against a story title, summary, and impact: it applies even when the source is official, and unsafe stories never backfill the target. `contextual_exclusions` adds rules with `anchor_keywords` and `context_keywords`; a rule excludes a story only when both groups match, so a broad anchor such as `China` does not exclude ordinary technology or industry coverage by itself. Material entity claims covering serious negative events, private financials, funding or valuation, M&A, IPO terms, and guarantees require an official source, while ordinary product and technical updates may keep one source. When enabled, manifest topics are deterministically rebuilt from the final XHS subset using a safe taxonomy; they do not inherit topics from omitted briefing categories. This adds no AI call, and `content-publisher` remains the final hard gate.
